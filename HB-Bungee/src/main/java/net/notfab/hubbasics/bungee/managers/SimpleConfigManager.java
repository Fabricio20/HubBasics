        ...
        if(config.contains("Logger")) {
            if(config.contains("Logger.Enabled"))
                HubBasics.getInstance().getLoggerManager()
                        .setEnabled(config.getBoolean("Logger.Enabled"));
            if(config.contains("Logger.Level")) {
                try {
                    HubBasics.getInstance().getLoggerManager()
                            .setLevel(Level.valueOf(config.getString("Logger.Level")));
                } catch (IllegalArgumentException ex) {
                    HubBasics.getInstance().getLoggerManager()
                            .error("Invalid logger Level found on config file", ex);
                }
            }
        }
    }
	...
	public void scan(String filePath) {
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				lineNumber++;
				if (line.contains("\t")) {
					HubBasics.getInstance().getProxy().getConsole().sendMessage(ChatColor.RED + " ------------------------------------------------------ ");
                    HubBasics.getInstance().getProxy().getConsole().sendMessage(ChatColor.RED + "HubBasics > Tab found in file \"" + filePath + "\" on line #" + lineNumber + "!");
                    HubBasics.getInstance().getProxy().getConsole().sendMessage(ChatColor.RED + " ------------------------------------------------------ ");
