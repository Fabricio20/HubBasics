...
public class VoidFallListener implements Listener {
	...
	public void Void(EntityDamageEvent event) {
		if(Main.getPlugin().getConfig().getBoolean("VoidFall.Enabled") == true) {
		if (event.getCause() == EntityDamageEvent.DamageCause.VOID) {
			...
			if ((ent instanceof Player)) {
				...
				((Player) ent).sendMessage(Main.getPlugin().getConfig().getString("VoidFall.Message").replace("&", "§").replace("%p", ((Player) ent).getName()));
